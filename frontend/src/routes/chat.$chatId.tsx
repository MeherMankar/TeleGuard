      ...
      <div className="min-h-screen bg-background text-foreground max-w-md mx-auto relative flex flex-col">
        ...
        <div className="sticky top-0 z-30 flex items-center gap-3 px-2 py-2 bg-card/95 backdrop-blur border-b border-border/40">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="w-10 h-10 rounded-full bg-sky-600 flex items-center justify-center text-white font-semibold shrink-0">
          ...
          <button aria-label="Search" className="p-2 text-foreground"><Search className="h-5 w-5" /></button>
          <button aria-label="More" className="p-2 text-foreground"><MoreVertical className="h-5 w-5" /></button>
        </div>
        ...
        >
          ...
          {sortedMessages.map((m, i) => {
            ...
            const showDate = !prev || (m.date && prev.date && new Date(m.date).toDateString() !== new Date(prev.date).toDateString())
            return (
              <div key={m.id}>
                {showDate && m.date && (
...
function MessageBubble({ message: m }: { message: Message }) {
  ...
  return (
    <div className={cn("flex", outgoing ? "justify-end" : "justify-start")}>
      <div className={cn("max-w-[80%] rounded-2xl overflow-hidden", outgoing ? "bg-sky-600 text-white rounded-br-md" : "bg-card text-foreground rounded-bl-md")}>
        {m.media && !m.text && (
        ...
        {m.text && (
          <div className="px-3 py-2">
            ...
            <div className="flex items-center justify-end gap-1 mt-0.5">
              <span className={cn("text-[11px]", outgoing ? "text-white/70" : "text-muted-foreground")}>{timeStr}</span>
              {outgoing && (
