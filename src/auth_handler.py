...
class OTPDestroyer:
    ...
    async def _cleanup_resources(self, client: Optional[TelegramClient], session_file: Optional[str]):
    ...
    async def verify_code(self, auth_data: Dict[str, any], code: str, password: Optional[str] = None) -> str:
...
class AuthManager:
    ...
    async def start_auth(self, user_id: int, phone: str, use_otp_destroyer: bool = False) -> bool:
    ...
    async def complete_auth(self, user_id: int, code: str, password: Optional[str] = None) -> str:
        ...
        try:
            if auth_info['type'] == 'destroy_mode':
                return await self._complete_destroy_mode(user_id, auth_info, code, password)
            else:
                return await self._complete_normal_auth(user_id, auth_info, code, password)
        except Exception as e:
            logger.error(f"Auth completion failed for user {user_id}: {e}")
            raise
    
    async def _complete_destroy_mode(self, user_id: int, auth_info: Dict, code: str, password: Optional[str]) -> str:
    ...
    async def _complete_normal_auth(self, user_id: int, auth_info: Dict, code: str, password: Optional[str]) -> str:
