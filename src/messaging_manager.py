...
from models import User, Account, MessageTemplate
...
class MessagingManager:
    ...
    async def setup_auto_reply(self, user_id: int, account_name: str, reply_message: str):
    ...
    async def forward_message(self, user_id: int, account_name: str, from_chat: str, to_chat: str, message_id: int) -> bool:
    ...
    async def get_templates(self, user_id: int) -> list:
        ...
        try:
            async with get_session() as session:
                ...
                return [(t.id, t.name, t.content) for t in templates]
        ...
        try:
            async with get_session() as session:
                ...
                result = await session.execute(
                    ...
                    .join(Account)
                    .join(User)
                    .where(User.telegram_id == user_id, MessageTemplate.id == template_id)
                )
