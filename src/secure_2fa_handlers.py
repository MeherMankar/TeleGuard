...
class Secure2FAHandlers:
    ...
    async def handle_secure_2fa_input(self, event, user_id: int, data: str):
        ...
        try:
            ...
            if action == "cancel":
                ...
                # Get session info
                session_info = self.account_manager.secure_input.pending_inputs.get(user_id)
                if not session_info:
                ...
                if session_type == "set_2fa_password":
                    await self.process_set_2fa_password(user_id, account_id, buffer, event.message_id)
                elif session_type == "change_2fa_current":
                    await self.process_change_2fa_current(user_id, account_id, buffer, event.message_id)
                elif session_type == "change_2fa_new":
                    await self.process_change_2fa_new(user_id, account_id, buffer, event.message_id)
                elif session_type == "remove_2fa_password":
                    await self.process_remove_2fa_password(user_id, account_id, buffer, event.message_id)
    ...
    async def process_set_2fa_password(self, user_id: int, account_id: int, password: str, message_id: int):
            ...
            async with get_session() as session:
                ...
                if success:
                    ...
                    await self.bot.edit_message(
                        user_id, message_id,
                        f"❌ **2FA Setup Failed**\n\n{result_msg}",
                        buttons=[[Button.inline("🔄 Try Again", f"2fa:set:{account_id}"), Button.inline("🔙 Back", "menu:2fa")]]
                    )
    ...
    async def process_change_2fa_current(self, user_id: int, account_id: int, current_password: str, message_id: int):
    ...
    async def process_change_2fa_new(self, user_id: int, account_id: int, new_password: str, message_id: int):
        ...
        try:
            ...
            async with get_session() as session:
                ...
                if success:
                    ...
                    hashed_password = self.account_manager.secure_2fa.hash_password_for_storage(new_password)
                    account.twofa_password = hashed_password
                    ...
                    await self.bot.edit_message(
                        user_id, message_id,
                        f"❌ **2FA Change Failed**\n\n{result_msg}",
                        buttons=[[Button.inline("🔄 Try Again", f"2fa:change:{account_id}"), Button.inline("🔙 Back", "menu:2fa")]]
                    )
    ...
    async def process_remove_2fa_password(self, user_id: int, account_id: int, password: str, message_id: int):
        ...
        try:
            async with get_session() as session:
                ...
                success, result_msg = await self.account_manager.secure_2fa.remove_2fa_password(
                    client, password
                )
                ...
                if success:
                    ...
                    await self.bot.edit_message(
                        ...
                        buttons=[[Button.inline("🔄 Try Again", f"2fa:remove:{account_id}"), Button.inline("🔙 Back", "menu:2fa")]]
                    )
