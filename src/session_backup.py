...
from mongo_store import store_session_temp, get_unpersisted_sessions, mark_session_persisted, log_audit_event

logger = logging.getLogger(__name__)

# Configuration
GIT_WORKDIR = os.environ.get('GIT_WORKDIR', '/tmp/sessions_repo')
...
class SessionBackupManager:
    ...
    def __init__(self):
        ...
        self.enabled = os.environ.get('SESSION_BACKUP_ENABLED', 'false').lower() == 'true'
    ...
    def ensure_local_clone(self):
        ...
        if not self.workdir.exists():
            subprocess.run(['git', 'clone', self.github_repo, str(self.workdir)], check=True)
            logger.info(f"Cloned sessions repo to {self.workdir}")
        else:
            subprocess.run(['git', '-C', str(self.workdir), 'fetch', 'origin'], check=True)
            subprocess.run(['git', '-C', str(self.workdir), 'reset', '--hard', 'origin/main'], check=True)
    ...
    def commit_and_push(self, message: str) -> str:
        ...
        subprocess.run(['git', '-C', str(self.workdir), 'commit', '-m', message], check=True)
        subprocess.run(['git', '-C', str(self.workdir), 'push', 'origin', 'main'], check=True)
    ...
    def push_sessions_batch(self) -> bool:
        ...
        try:
            ...
            # Commit and push
            commit_sha = self.commit_and_push(f"Session backup @ {datetime.now(timezone.utc).isoformat()}Z")
            ...
            if commit_sha:
                ...
                for session in sessions:
                    ...
                    log_audit_event(
                        session['account_id'],
                        'push_to_github',
                        {'commit': commit_sha, 'manifest_version': manifest['generated_at']},
                        'scheduler'
                    )
            ...
            log_audit_event(
                None,
                'push_to_github',
                {'error': str(e), 'session_count': len(sessions) if 'sessions' in locals() else 0},
                'scheduler'
            )
            return False
    ...
    def compact_history(self) -> bool:
        ...
        try:
            ...
            subprocess.run(['git', '-C', str(self.workdir), 'checkout', '-B', backup_branch], check=True)
            subprocess.run(['git', '-C', str(self.workdir), 'push', 'origin', backup_branch], check=True)
            
            # Create orphan branch with current files
            subprocess.run(['git', '-C', str(self.workdir), 'checkout', '--orphan', 'clean_main'], check=True)
            subprocess.run(['git', '-C', str(self.workdir), 'rm', '-rf', '.'], check=True)
            
            # Re-add current files
            if (self.workdir / 'sessions').exists():
                subprocess.run(['git', '-C', str(self.workdir), 'add', 'sessions'], check=True)
            if (self.workdir / 'manifests').exists():
                subprocess.run(['git', '-C', str(self.workdir), 'add', 'manifests'], check=True)
            
            subprocess.run(['git', '-C', str(self.workdir), 'commit', '-m', f'compact snapshot @ {ts}'], check=True)
            
            # Force-push to main
            subprocess.run(['git', '-C', str(self.workdir), 'push', '-f', 'origin', 'clean_main:main'], check=True)
            
            # Clean up local branch
            subprocess.run(['git', '-C', str(self.workdir), 'checkout', 'main'], check=True)
            subprocess.run(['git', '-C', str(self.workdir), 'branch', '-D', 'clean_main'], check=True)
