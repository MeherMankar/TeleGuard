...
async def send_profile_management(bot, user_id, account_id, message_id, account_manager):
    ...
    if not account:
        ...
        return
    current_info = await _get_current_profile_info(user_id, account["name"], account_manager)
    if current_info:
...
async def _get_current_profile_info(user_id, account_name, account_manager):
    try:
        if not account_manager or user_id not in account_manager.user_clients:
            return None
        user_clients = account_manager.user_clients.get(user_id, {})
...
async def handle_profile_name_change(bot, user_id, account_id, event, account_manager):
    account = await mongodb.db.accounts.find_one({"_id": ObjectId(account_id), "user_id": user_id})
    if not account:
        await event.answer("Account not found")
        return
    await event.answer("Enter new name")
    await bot.send_message(user_id, "Reply with your new name (Example: John Doe):")
    ...
    async def handle_name_input(name_event):
        ...
        client = None
        if hasattr(account_manager, "user_clients"):
            client = account_manager.user_clients.get(user_id, {}).get(account["name"])
        if client:
            try:
        ...
        bot.remove_event_handler(handle_name_input)

async def handle_profile_username_change(bot, user_id, account_id, event, account_manager):
    account = await mongodb.db.accounts.find_one({"_id": ObjectId(account_id), "user_id": user_id})
    if not account:
        await event.answer("Account not found")
        return
    await event.answer("Enter username")
    await bot.send_message(user_id, "Reply with your new username (without @):")
    @bot.on(events.NewMessage(from_users=user_id))
    async def handle_username_input(username_event):
        ...
        bot.remove_event_handler(handle_username_input)

async def handle_profile_bio_change(bot, user_id, account_id, event, account_manager):
    account = await mongodb.db.accounts.find_one({"_id": ObjectId(account_id), "user_id": user_id})
    if not account:
        await event.answer("Account not found")
        return
    await event.answer("Enter bio")
    await bot.send_message(user_id, "Reply with your new bio (max 70 characters):")
    @bot.on(events.NewMessage(from_users=user_id))
    async def handle_bio_input(bio_event):
        ...
        bot.remove_event_handler(handle_bio_input)

async def handle_profile_photo_change(bot, user_id, account_id, event, account_manager):
    account = await mongodb.db.accounts.find_one({"_id": ObjectId(account_id), "user_id": user_id})
    if not account:
        await event.answer("Account not found")
        return
    await event.answer("Send photo")
    await bot.send_message(user_id, "Send a photo to set as your profile picture:")
    @bot.on(events.NewMessage(from_users=user_id, func=lambda e: e.photo))
    async def handle_photo_input(photo_event):
        ...
        bot.remove_event_handler(handle_photo_input)
