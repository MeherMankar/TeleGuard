"""Split from join_channel.py - channel_join"""
"""
        ...
        try:
            ...
            try:
                participant = await client(functions.channels.GetParticipantRequest(
                    ...
                    participant=client.get_me()
                ))
                
                # Check if user has delete permissions
                if not (hasattr(participant.participant, 'admin_rights') and 
                       getattr(participant.participant.admin_rights, 'delete_messages', False)) and \
                   not isinstance(participant.participant, types.ChannelParticipantCreator):
            ...
            result = await client(functions.channels.DeleteChannelRequest(channel_entity))
