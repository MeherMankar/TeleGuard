        ...
        async with get_session() as session:
            ...
            for account in accounts:
                try:
                    ...
                    logger.info(f"Migrated session for account {account.name} ({account.phone})")
                    migrated_count += 1
                    
                except Exception as e:
                    logger.error(f"Failed to migrate account {account.name}: {e}")
            
            logger.info(f"Migration completed: {migrated_count}/{len(accounts)} accounts migrated")
            
            if migrated_count > 0:
                logger.info("Sessions are now stored in MongoDB and will be backed up to GitHub on next scheduled run")
...
async def verify_migration():
    ...
    try:
        ...
        logger.info(f"Verification: {sqlite_count} accounts in SQLite, {mongo_count} sessions in MongoDB")
