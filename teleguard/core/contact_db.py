...
class ContactDB:
    ...
    async def add_contact(contact: Contact) -> bool:
        """Add new contact"""
        try:
            await mongodb.db.contacts.create_index(
                [("user_id", 1), ("managed_by_account", 1)], unique=True
            )
    ...
    async def get_contact(user_id: int, managed_by_account: str) -> Optional[Contact]:
        ...
        try:
            data = await mongodb.db.contacts.find_one(
    ...
    async def update_contact(
        user_id: int, managed_by_account: str, update_data: dict
    ) -> bool:
        """Update contact"""
        try:
    ...
    async def delete_contact(user_id: int, managed_by_account: str) -> bool:
        """Delete contact"""
        try:
            result = await mongodb.db.contacts.delete_one(
                {"user_id": user_id, "managed_by_account": managed_by_account}
            )
    ...
    async def get_all_contacts(
        managed_by_account: str, limit: int = 100
    ) -> List[Contact]:
        """Get all contacts for account"""
        try:
            cursor = mongodb.db.contacts.find(
    ...
    async def get_contacts_by_tag(managed_by_account: str, tag: str) -> List[Contact]:
        ...
        try:
            ...
            cursor = mongodb.db.contacts.find(
                {"managed_by_account": managed_by_account, "tags": safe_tag}
            ).limit(50)
            contacts = []
            async for data in cursor:
                contacts.append(Contact.from_dict(data))
    ...
    @staticmethod
    async def set_contact_blacklist_status(
        user_id: int, managed_by_account: str, status: bool
    ) -> bool:
        """Set blacklist status"""
        return await ContactDB.update_contact(
            user_id, managed_by_account, {"is_blacklisted": status}
        )

    @staticmethod
    async def set_contact_whitelist_status(
        user_id: int, managed_by_account: str, status: bool
    ) -> bool:
        """Set whitelist status"""
        return await ContactDB.update_contact(
            user_id, managed_by_account, {"is_whitelisted": status}
        )
    ...
    async def add_group(group: ContactGroup) -> bool:
        """Add new group"""
        try:
            await mongodb.db.contact_groups.create_index(
                [("name", 1), ("managed_by_account", 1)], unique=True
            )
    ...
    async def get_group(name: str, managed_by_account: str) -> Optional[ContactGroup]:
        """Get group by name"""
        try:
            data = await mongodb.db.contact_groups.find_one(
                {"name": name, "managed_by_account": managed_by_account}
            )
    ...
    async def add_contact_to_group(
        user_id: int, group_name: str, managed_by_account: str
    ) -> bool:
    ...
    async def get_all_groups(managed_by_account: str) -> List[ContactGroup]:
        ...
        try:
            cursor = mongodb.db.contact_groups.find(
                {"managed_by_account": managed_by_account}
            )
            groups = []
            async for data in cursor:
