...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.operations: Dict[str, Dict[str, list]] = defaultdict(
    ...
    def _cleanup_old_operations(self, account_phone: str, operation: str):
        ...
        if (
            account_phone in self.operations
            and operation in self.operations[account_phone]
        ):
            self.operations[account_phone][operation] = [
                (ts, count)
                for ts, count in self.operations[account_phone][operation]
                if ts > cutoff_time
            ]
    ...
    def _get_operation_count(self, account_phone: str, operation: str) -> int:
        ...
        if (
            account_phone not in self.operations
            or operation not in self.operations[account_phone]
        ):
            return 0
