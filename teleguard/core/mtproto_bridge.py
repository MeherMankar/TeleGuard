...
class MTProtoBridge:
    ...
    async def create_bridge(self, proxy_id: str, mtproto_config: Dict) -> Optional[Tuple[str, int]]:
        """
        ...
        try:
            ...
            logger.info(f"MTProto bridge created: 127.0.0.1:{bridge_port} -> {mtproto_config['addr']}:{mtproto_config['port']}")
    ...
    async def _start_bridge_process(self, local_port: int, remote_host: str, remote_port: int, secret: bytes) -> Optional[asyncio.subprocess.Process]:
