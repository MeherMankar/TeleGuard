...
class SessionLock:
    ...
    def __enter__(self):
        if not self.acquire():
            raise RuntimeError(f"Cannot acquire session lock. Another process may be using {self.session_path}")
        return self
...
class SessionGuardian:
    ...
    def log_action(self, action: str, target: str, success: bool = True, error: str = None):
    ...
    async def handle_auth_key_error(self, exception: Exception, context: Dict[str, Any] = None):
        ...
        error_report = {
            ...
            'context': context or {},
            'last_actions': [asdict(action) for action in list(self.action_history)[-10:]]
        }
        
        # Log detailed error
        logger.critical(f"Session revocation details: {json.dumps(error_report, indent=2)}")
    ...
    async def exponential_backoff(self, func: Callable, *args, max_attempts: int = 5, base_delay: float = 0.5, **kwargs):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.is_auth_key_error(e):
                    await self.handle_auth_key_error(e, {
                ...
                if attempt == max_attempts - 1:
                    self.log_action(f"retry_failed_{func.__name__}", str(args)[:50], False, str(e))
                    raise e
    ...
    async def _handle_ip_change(self, old_ip: str, new_ip: str):
        ...
        if self.config.get('ip_monitoring', {}).get('notify_on_change', True):
            location = f"{geo_data.get('city', 'Unknown')}, {geo_data.get('country', 'Unknown')}" if geo_data else 'Unknown'
            message = (
    ...
    async def send_periodic_summary(self):
        ...
        try:
            current_time = time.time()
            if current_time - self.last_summary_time < self.config.get('summary_interval', 36000):  # 10 hours
                return
            ...
            for name, limiter in self.rate_limiters.items():
                summary += f"• {name}: {limiter.tokens:.1f}/{limiter.max_tokens} tokens\n"
            ...
            if self.config.get('ip_monitoring', {}).get('enabled', True):
                ...
                summary += f"• Stability: {'High' if self.ip_change_count < 3 else 'Medium' if self.ip_change_count < 10 else 'Low'}\n"
