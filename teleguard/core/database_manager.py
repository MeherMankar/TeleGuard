...
from .redis_cache import redis_cache, init_redis
from .mongo_database import mongodb, init_db
...
class DatabaseManager:
    ...
    # Rate Limiting (Redis)
    async def check_rate_limit(self, user_id: int, endpoint: str = "default", 
                             limit: int = 30, window: int = 60) -> bool:
        """Check rate limit using Redis cache"""
        return await self.rate_limiter.check_rate_limit(user_id, endpoint, limit, window)
    async def get_rate_limit_remaining(self, user_id: int, endpoint: str = "default",
                                     limit: int = 30, window: int = 60) -> int:
        """Get remaining rate limit requests"""
        return await self.rate_limiter.get_remaining_requests(user_id, endpoint, limit, window)
    ...
    async def delete_account(self, account_id: str):
        ...
        if account and 'user_id' in account:
            try:
                ...
                logger.warning(f"Failed to remove 2FA password for account {account_id}: {e}")
    ...
    async def cache_multiple_items(self, items: Dict[str, tuple], default_ttl: int = 3600):
