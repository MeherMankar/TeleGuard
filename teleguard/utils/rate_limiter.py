...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        if redis_cache.connected:
            try:
                ...
                if not allowed:
                    remaining = await redis_cache.get_rate_limit_remaining(key, limit, window)
    ...
    async def _memory_rate_limit(self, key: str, limit: int, window: int) -> bool:
        ...
        # Clean old requests
        self.fallback_requests[key] = [
            req for req in self.fallback_requests[key] 
        ...
        return True
    async def get_remaining_requests(self, user_id: int, endpoint: str = "default", limit: int = 30, window: int = 60) -> int:
