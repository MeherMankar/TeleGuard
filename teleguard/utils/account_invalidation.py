...
class AccountInvalidationHandler:
    ...
    async def handle_account_invalidation(
        ...
        try:
            success = await self._remove_invalidated_account(
                user_id, account_name, phone
            )
            if success:
    ...
    async def _cleanup_session_monitor(self, user_id: int, account_name: str) -> None:
        ...
        if (
            hasattr(self.bot_manager, "session_monitor")
            and self.bot_manager.session_monitor
        ):
            self.bot_manager.session_monitor.remove_client_from_monitor(
    ...
    async def _send_invalidation_notification(
        ...
        try:
            message = self._build_notification_message(
                account_name, phone, error_message
            )
    ...
    def _build_notification_message(
        self, account_name: str, phone: str, error_message: str
    ) -> str:
