...
class Secure2FAHandlers:
    ...
    async def handle_secure_2fa_input(self, event, user_id: int, data: str) -> None:
        ...
        try:
            ...
            if not self._is_service_available():
                ...
                return
            completed, buffer, action = self.account_manager.secure_input.handle_keypad_input(user_id, key)
    ...
    def _is_service_available(self) -> bool:
        ...
        return (
            self.account_manager is not None and
            hasattr(self.account_manager, 'secure_input') and
            hasattr(self.account_manager, 'secure_2fa')
        )
    async def _process_keypad_action(self, event, user_id: int, action: str, buffer: str, completed: bool) -> None:
    ...
    async def _route_to_processor(self, event, user_id: int, session_info: Dict[str, Any], buffer: str) -> None:
    ...
    async def _get_account(self, user_id: int, account_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Database error getting account {account_id} for user {user_id}: {e}")
            return None
    ...
    async def _send_error_message(self, user_id: int, message_id: int, title: str, error: str) -> None:
    ...
    async def _log_audit_event(self, account_id: str, action: str, user_id: int, success: bool) -> None:
    ...
    async def process_set_2fa_password(self, user_id: int, account_id: int, password: str, message_id: int) -> None:
        """Process setting new 2FA password with comprehensive validation"""
        try:
            account = await self._get_account(user_id, account_id)
            if not account:
                await self._send_error_message(user_id, message_id, "Account Not Found", "Account not found in database.")
                return
            client = self._get_client(user_id, account.get('name'))
            if not client:
                await self._send_error_message(user_id, message_id, "Client Not Found", "Please restart the bot.")
                return
            success, result_msg = await self.account_manager.secure_2fa.set_2fa_password(
                client, password, hint=self.PASSWORD_HINT
            )
            if success:
                await self._handle_2fa_success(user_id, account, password, message_id, "set", result_msg)
            else:
                await self._handle_2fa_failure(user_id, message_id, "2FA Setup Failed", result_msg, account_id, "set")
        except Exception as e:
            logger.error(f"2FA set password error for user {user_id}: {e}")
            await self._send_error_message(user_id, message_id, "Unexpected Error", f"{type(e).__name__}")
    async def _handle_2fa_success(self, user_id: int, account: Dict[str, Any], password: str, 
                                 message_id: int, action: str, result_msg: str) -> None:
        """Handle successful 2FA operation"""
        hashed_password = self.account_manager.secure_2fa.hash_password_for_storage(password)
        await mongodb.db.accounts.update_one(
            {"_id": account["_id"]},
            {"$set": {"twofa_password": hashed_password}}
        )
        await self._log_audit_event(account["_id"], f"{action}_2fa_password", user_id, True)
        action_text = {
            "set": "Set",
            "change": "Changed", 
            "remove": "Removed"
        ...
        await self.bot.edit_message(
            user_id,
            message_id,
            f"✅ **2FA Password {action_text}**\n\n{result_msg} for {account.get('name')}",
            buttons=[[Button.inline("🔙 Back to 2FA", "menu:2fa")]]
        )
    async def _handle_2fa_failure(self, user_id: int, message_id: int, title: str, 
                                 error_msg: str, account_id: int, action: str) -> None:
    ...
    async def process_change_2fa_current(self, user_id: int, account_id: int, current_password: str, message_id: int) -> None:
        """Process current password for 2FA change"""
        if not self.account_manager:
            await self._send_error_message(user_id, message_id, "Service Error", "Account manager unavailable.")
            return
        ...
        self.account_manager.secure_input.start_secure_input(user_id, "change_2fa_new", account_id)
        # Refresh session timestamp
        if user_id in self.account_manager.secure_input.pending_inputs:
            self.account_manager.secure_input.pending_inputs[user_id]["started_at"] = time.time()
    ...
    async def process_change_2fa_new(self, user_id: int, account_id: int, new_password: str, message_id: int) -> None:
        ...
        try:
            ...
            if not current_password:
                await self._send_error_message(user_id, message_id, "Session Expired", "Please start over.")
                return
            account = await self._get_account(user_id, account_id)
            if not account:
                await self._send_error_message(user_id, message_id, "Account Not Found", "Account not found in database.")
            ...
            if not client:
                await self._send_error_message(user_id, message_id, "Client Not Found", "Please restart the bot.")
                return
            success, result_msg = await self.account_manager.secure_2fa.change_2fa_password(
                client, current_password, new_password, hint="Changed via TeleGuard Bot"
            )
            if success:
                await self._handle_2fa_success(user_id, account, new_password, message_id, "change", result_msg)
            else:
                await self._handle_2fa_failure(user_id, message_id, "2FA Change Failed", result_msg, account_id, "change")
            # Clear pending action
            self.account_manager.pending_actions.pop(user_id, None)
        except Exception as e:
            logger.error(f"2FA change password error for user {user_id}: {e}")
            await self._send_error_message(user_id, message_id, "Unexpected Error", f"{type(e).__name__}")
    async def process_remove_2fa_password(self, user_id: int, account_id: int, password: str, message_id: int) -> None:
        ...
        try:
            ...
            if not client:
                await self._send_error_message(user_id, message_id, "Client Not Found", "Please restart the bot.")
                return
            success, result_msg = await self.account_manager.secure_2fa.remove_2fa_password(client, password)
            if success:
                ...
                await self._log_audit_event(account["_id"], "remove_2fa_password", user_id, True)
                await self.bot.edit_message(
                    user_id,
                    message_id,
                ...
                await self._handle_2fa_failure(user_id, message_id, "2FA Removal Failed", result_msg, account_id, "remove")
        except Exception as e:
            logger.error(f"2FA remove password error for user {user_id}: {e}")
            await self._send_error_message(user_id, message_id, "Unexpected Error", f"{type(e).__name__}")
    async def show_2fa_status(self, user_id: int, account_id: int, message_id: int) -> None:
        ...
        try:
            ...
            if not client:
                await self._send_error_message(user_id, message_id, "Client Not Found", "Please restart the bot.")
                return
            success, status_info = await self.account_manager.secure_2fa.check_2fa_status(client)
            if success:
                await self._display_2fa_status(user_id, message_id, account, status_info, account_id)
            else:
                await self._send_error_message(user_id, message_id, "Cannot Check 2FA Status", "Client connection issue.")
        except Exception as e:
            logger.error(f"2FA status check error for user {user_id}: {e}")
            await self._send_error_message(user_id, message_id, "Status Check Failed", f"{type(e).__name__}")
    async def _display_2fa_status(self, user_id: int, message_id: int, account: Dict[str, Any], 
                                 status_info: Dict[str, Any], account_id: int) -> None:
    ...
    def _build_2fa_status_buttons(self, has_password: bool, account_id: int) -> list:
        ...
        if has_password:
            ...
            buttons.append([Button.inline("➕ Set 2FA Password", f"2fa:set:{account_id}")])
