...
class ActivityLogger:
    ...
    async def get_user_activity(user_id: int, limit: int = 100):
        ...
        try:
            activities = (
                await mongodb.db.activity_logs.find({"user_id": user_id})
                .sort("timestamp", -1)
                .limit(limit)
    ...
    async def log_account_action(user_id: int, account_name: str, action: str):
        ...
        await ActivityLogger.log_action(
            user_id,
            action,
            {"account": account_name, "action_type": "account_management"},
        )
    ...
    async def log_security_action(user_id: int, action: str, severity: str = "info"):
        ...
        await ActivityLogger.log_action(
            user_id, action, {"action_type": "security", "severity": severity}
        )
