...
class SessionProtection:
    ...
    async def check_message_safety(
        self, session_id: str, message_content: str = "", target: str = ""
    ) -> bool:
        ...
        # Check rate limits
        if limits["messages_per_hour"] >= self._get_message_limit(
            tracker["protection_level"]
        ):
        ...
        if time_since_last < min_delay:
            ...
            logger.info(
                f"Rate limiting message for {
                    tracker['account_name']}, waiting {
                    wait_time:.1f}s"
            )
    ...
    def increase_protection_level(self, session_id: str, reason: str = ""):
        ...
        logger.warning(
            f"Increased protection level to {
                tracker['protection_level']} for {
                tracker['account_name']}: {reason}"
        )
    ...
    def _is_suspicious_message(self, content: str) -> bool:
        ...
        suspicious_count = sum(
            1 for pattern in suspicious_patterns if pattern in content_lower
        )
