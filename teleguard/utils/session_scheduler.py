...
class SessionScheduler:
    ...
    def __init__(self, bot_client=None, bot_manager=None):
    ...
    def trigger_user_settings_push_now(self):
        ...
        if self.running and self.bot_client:
            self.scheduler.add_job(
                self._push_user_settings_job,
                id="manual_user_settings_push",
                name="Manual user settings push",
            )
    ...
    def trigger_user_ids_push_now(self):
        ...
        if self.running and self.bot_client:
            self.scheduler.add_job(
                self._push_user_ids_job,
                id="manual_user_ids_push",
                name="Manual user IDs push",
            )
    ...
    def trigger_session_files_push_now(self):
        ...
        if self.running and self.bot_client:
            self.scheduler.add_job(
                self._push_session_files_job,
                id="manual_session_files_push",
                name="Manual session files push",
            )
    ...
    async def _push_user_settings_job(self):
        ...
        try:
            ...
            success = await self.backup_manager.push_user_settings_to_telegram(
                self.bot_client
            )
            if success:
    ...
    async def _push_user_ids_job(self):
        ...
        try:
            ...
            success = await self.backup_manager.push_user_ids_to_telegram(
                self.bot_client
            )
            if success:
    ...
    async def _push_session_files_job(self):
        ...
        try:
            ...
            success = await self.backup_manager.push_session_files_to_telegram(
                self.bot_client
            )
            if success:
    ...
    async def schedule_task(
        ...
        try:
            task_id = (
                f"{task_type}_{user_id}_{account_id}_{int(datetime.now().timestamp())}"
            )
            ...
            task_info = {
                ...
                "repeat": repeat,
                "repeat_interval": (
                    repeat_interval.total_seconds() if repeat_interval else None
                ),
        ...
        try:
            ...
            for task_id, task_info in self.scheduled_tasks.items():
                if task_info["user_id"] == user_id:
                    task_status = (
                        "running" if task_id in self.running_tasks else "scheduled"
                    )
                    user_tasks.append({**task_info, "status": task_status})
