...
class TelegramBackup:
    ...
    def __init__(self, bot_client):
        self.bot = bot_client
        self.channel_id = int(TELEGRAM_BACKUP_CHANNEL) if TELEGRAM_BACKUP_CHANNEL else None
    ...
    async def backup_user_settings(self):
        ...
        try:
            ...
            settings_data = []
            for user in users:
                user_settings = await db_manager.get_user_settings(user.get('telegram_id'))
                if user_settings:
            ...
            if settings_data:
                ...
                filename = f"user_settings_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                await self.bot.send_message(self.channel_id, "📝 No user settings found to backup")
    ...
    async def backup_user_ids(self):
        ...
        try:
            ...
            if user_ids:
                ...
                filename = f"user_ids_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                await self.bot.send_message(self.channel_id, "📝 No user IDs found to backup")
    ...
    async def backup_session_files(self):
        ...
        try:
            ...
            if session_info:
                ...
                filename = f"session_info_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                await self.bot.send_message(self.channel_id, "📝 No session info found to backup")
    ...
    async def _send_backup(self, data):
        ...
        try:
            ...
            await self.bot.send_file(
                ...
                content.encode(),
                file_name=filename,
                caption=f"🔄 Auto Backup: {data['action']}\n📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
