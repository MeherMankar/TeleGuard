        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                if "user_id" in key_pattern and len(args) > 0:
                    user_id = args[0] if isinstance(args[0], int) else kwargs.get('user_id')
                    if user_id:
                        await db_manager.redis.invalidate_user_cache(user_id)
                
                if "account_id" in key_pattern and len(args) > 0:
                    account_id = args[0] if isinstance(args[0], str) else kwargs.get('account_id')
                    if account_id:
    ...
    async def bulk_cache_accounts(account_ids: list):
        ...
        try:
            ...
            for account_id in account_ids:
                ...
                if account:
                    cache_items[f"account_data:{account_id}"] = (account, 1200)  # 20min TTL
...
async def cache_api_response(endpoint: str, params: dict, response: Any, ttl: int = 300):
