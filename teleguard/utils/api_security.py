...
class RateLimiter:
    ...
    async def check_rate_limit(self, user_id: int) -> tuple:
        ...
        if user_id not in self._usage_cache:
            self._usage_cache[user_id] = {"requests": [], "last_cleanup": current_time}
    ...
    def cleanup_old_data(self) -> None:
        ...
        for user_id in list(self._usage_cache.keys()):
            user_data = self._usage_cache[user_id]
            user_data["requests"] = [
                req_time for req_time in user_data["requests"] if req_time > cutoff_time
            ]
...
async def validate_api_key(api_key: str) -> Optional[int]:
    ...
    try:
        ...
        account = await mongodb.db.accounts.find_one(
            {"user_id": user_id, "api_access_enabled": True, "api_key_hash": key_hash}
        )
        if account:
            await _log_security_event(
                user_id,
                "api_access_granted",
                {"account_id": str(account["_id"]), "remaining_requests": remaining},
            )
            return user_id
        else:
...
async def generate_user_api_key(user_id: int, account_id: str) -> Optional[str]:
    ...
    try:
        ...
        if result.modified_count > 0:
            await _log_security_event(
                user_id, "api_key_generated", {"account_id": account_id}
            )
...
async def revoke_user_api_key(user_id: int, account_id: str) -> bool:
    ...
    try:
        ...
        if result.modified_count > 0:
            await _log_security_event(
                user_id, "api_key_revoked", {"account_id": account_id}
            )
...
async def get_api_usage_stats(user_id: int) -> Dict[str, Any]:
    ...
    try:
        allowed, remaining, reset_time = await rate_limiter.check_rate_limit(user_id)
        account = await mongodb.db.accounts.find_one(
            {"user_id": user_id, "api_access_enabled": True}
        )
        if not account:
            return {"api_enabled": False, "error": "No API access enabled"}
        return {
            "api_enabled": True,
        ...
        return {"api_enabled": False, "error": f"Failed to get usage stats: {str(e)}"}


async def _log_security_event(
    user_id: int, event_type: str, details: Dict[str, Any]
) -> None:
    """
