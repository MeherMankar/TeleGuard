...
def load_guardian_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        # Emergency settings
        'emergency': {
            'auto_shutdown_on_auth_error': os.getenv('AUTO_SHUTDOWN_ON_AUTH_ERROR', 'true').lower() == 'true',
            'delete_session_on_revoke': os.getenv('DELETE_SESSION_ON_REVOKE', 'false').lower() == 'true',
            'notify_admin_on_error': os.getenv('NOTIFY_ADMIN_ON_ERROR', 'true').lower() == 'true',
        },
        ...
        'ip_monitoring': {
            ...
            'notify_on_change': os.getenv('NOTIFY_ON_IP_CHANGE', 'true').lower() == 'true',
            'session_warmup_delay': int(os.getenv('SESSION_WARMUP_DELAY', '30')),  # 30 seconds
        }
    }
