...
class ChannelSearch:
    ...
    async def search_channels(self, user_id: int, account_name: str, query: str, limit: int = 20):
    ...
    async def get_popular_channels(self, category: str = "all"):
        ...
        try:
            ...
            popular_channels = [
                {'title': 'Telegram News', 'username': 'telegram', 'type': 'channel', 'category': 'news'},
                {'title': 'Tech Updates', 'username': 'techupdates', 'type': 'channel', 'category': 'tech'},
                {'title': 'Crypto Signals', 'username': 'cryptosignals', 'type': 'channel', 'category': 'crypto'},
                {'title': 'Programming Tips', 'username': 'programmingtips', 'type': 'channel', 'category': 'tech'},
                {'title': 'Daily News', 'username': 'dailynews', 'type': 'channel', 'category': 'news'}
            ]
            
            if category != "all":
                popular_channels = [ch for ch in popular_channels if ch.get('category') == category]
