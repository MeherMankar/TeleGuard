...
class OTPMetrics:
    ...
    async def record_block(
        account_id: str, reason: str = "otp_destroyed", metadata: Dict = None
    ) -> bool:
        """
        ...
        try:
            ...
            await mongodb.db.otp_stats.update_one(
                {"account_id": account_id, "date": date},
                {
                    ...
                    "$set": {"last_block": datetime.utcnow(), "last_reason": reason},
                    "$push": {
                        "recent_blocks": {
                            "$each": [
                                {
    ...
    async def record_allow(
        account_id: str, reason: str = "temp_passthrough", metadata: Dict = None
    ) -> bool:
        """
    ...
    async def get_global_stats(start_date: str = None, end_date: str = None) -> Dict:
        ...
        try:
            ...
            # Get top accounts
            top_accounts = await OTPMetrics.get_top_blocked_accounts(
    ...
    @staticmethod
    async def get_top_blocked_accounts(
    ...
    async def get_activity_sparkline(days: int) -> str:
        ...
        try:
            ...
            pipeline = [
                ...
                {"$group": {"_id": "$date", "total": {"$sum": "$blocked_count"}}},
                {"$sort": {"_id": 1}},
            ]
