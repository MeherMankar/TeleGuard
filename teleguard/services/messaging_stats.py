...
class MessagingStats:
    ...
    async def record_message_sent(
        self, user_id: int, account_id: str, target: str, message_type: str = "message"
    ):
    ...
    async def record_template_used(
        self, user_id: int, account_id: str, template_name: str
    ):
    ...
    async def get_recent_messages(self, user_id: int, limit: int = 20):
        ...
        try:
            messages = (
                await self.db.messages_log.find({"user_id": user_id})
                .sort("timestamp", -1)
                .limit(limit)
