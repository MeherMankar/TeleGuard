        ...
        try:
            ...
            if target and message_template:
                ...
                if config.get('ai_enhance', False) and self.ai_model:
                    enhanced_message = await self._ai_enhance_message(message_template, target)
        ...
        if hasattr(self.bot_manager, "fullclient_manager"):
            ...
            if account:
                ...
                if random.random() < 0.3:  # 30% chance for additional activity
                    await self._simulate_natural_online_activity(account["user_id"], account_id)
        ...
        try:
            ...
            for target in targets:
                try:
                    ...
                    for message in messages:
                        if (message.sender_id != me.id and message.text and 
                            ...
                            if should_reply:
                                reply = await self._ai_generate_contextual_reply(message, target)
                                if reply:
            ...
            for group_id in groups:
                try:
                    ...
                    for message in messages:
                        ...
                        # Check for mentions
                        if ('mention_response' in engagement_types and me.username and 
                            f'@{me.username}' in (message.text or '').lower()):
...
Respond with only 'yes' or 'no':"""
    ...
    async def _ai_generate_contextual_reply(self, message, chat_id) -> Optional[str]:
        ...
        try:
            ...
            context = "\n".join([f"{msg.sender_id}: {msg.text or '[media]'}" for msg in recent_messages if msg.text])
...
Reply:"""
            ...
            # Validate reply
            if len(reply_text) > 200 or any(word in reply_text.lower() for word in ['ai', 'artificial', 'bot']):
                return None
        ...
        try:
            ...
            if dialogs:
                ...
                if random.random() < 0.4:  # 40% chance to send
                    casual_responses = [
                        "👍", "ok", "got it", "thanks", "sure", "alright", 
                        "sounds good", "will do", "understood", "✅"
                    ]
                    await client.send_message(dialog.entity, random.choice(casual_responses))
