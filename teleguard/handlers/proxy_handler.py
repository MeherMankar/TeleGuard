...
class ProxyHandler:
    ...
    async def _show_proxy_list(self, event, user_id):
        ...
        for proxy in proxies[:10]:  # Show first 10
            ...
            response_time = (
                f" ({proxy['response_time']}s)" if proxy.get("response_time") else ""
            )
    ...
    async def _start_add_proxy(self, event, user_id):
        ...
        await self._safe_edit(
            event, text, buttons=[[Button.inline("❌ Cancel", "proxy:menu")]]
        )
    ...
    async def process_proxy_input(self, event, user_id, text):
        ...
        try:
            ...
            # Add proxy
            await processing_msg.edit(
                f"📥 Adding proxy {proxy_data['server']}:{proxy_data['port']}..."
            )
            ...
            if success:
                ...
                # Test proxy
                test_success, test_msg, response_time = await proxy_manager.test_proxy(
                    proxy_id, self.bot_manager
                )
                ...
                if test_success:
                        ...
                        await self.bot.send_message(
                            ...
                            buttons=[
                                [
                                    Button.inline(
                                        "🔗 Assign to Account",
                                        f"proxy:assign_new:{proxy_id}",
                                    )
                                ],
                                [
                                    Button.inline(
                                        "🤖 Set as Default for New Accounts",
                                        f"proxy:set_default:{proxy_id}",
                                    )
                                ],
                                [Button.inline("📋 View Proxies", "proxy:list")],
                            ],
                    ...
                    await self.bot.send_message(
                        ...
                        buttons=[
                            [Button.inline("📋 View Proxies", "proxy:list")],
                            [Button.inline("🧪 Test Again", f"proxy:test:{proxy_id}")],
                        ],
                    )
            else:
                await processing_msg.edit(
    ...
    async def _test_proxy(self, event, user_id, proxy_id):
        ...
        success, message, response_time = await proxy_manager.test_proxy(
            proxy_id, self.bot_manager
        )
        ...
        try:
            if success:
                await event.answer(
                    f"✅ Proxy working! ({response_time:.2f}s)", alert=True
                )
            else:
    ...
    async def _show_account_selection(self, event, user_id):
        ...
        for account in accounts:
            ...
            buttons.append(
                [
                    Button.inline(
                        f"{has_proxy} {display_name}", f"proxy:assign_to:{account_id}"
                    )
                ]
            )
    ...
    async def _show_proxy_selection_for_account(self, event, user_id, account_id):
        ...
        if current_proxy_id:
            buttons.append(
                [Button.inline("🚫 Remove Proxy", f"proxy:remove:{account_id}")]
            )
    ...
    async def _assign_proxy(self, event, user_id, account_id, proxy_id):
        ...
        new_proxy = await mongodb.db.proxies.find_one(
            {"_id": ObjectId(proxy_id), "user_id": user_id}
        )

        success, message = await proxy_manager.assign_proxy_to_account(
            user_id, account_id, proxy_id
        )
        ...
        if success:
            ...
            if phone and phone in self.bot_manager.user_clients:
                try:
                    ...
                    await event.answer(
                        "✅ Proxy assigned! Restart account manually.", alert=True
                    )
            else:
                if account and new_proxy:
                    account_name = account.get("name", "Unknown")
                    ...
                    await event.answer(
                        "✅ Proxy assigned! Start account to apply.", alert=True
                    )
        else:
            try:
                await event.answer(f"❌ {message}", alert=True)
        ...
        if success:
            ...
            if phone and phone in self.bot_manager.user_clients:
                try:
                    ...
                    logger.error(f"Restart error: {e}")
                    await event.answer(
                        "✅ Proxy removed! Restart account manually.", alert=True
                    )
            else:
                if account:
                    account_name = account.get("name", "Unknown")
                    ...
                    await event.answer(
                        "✅ Proxy removed! Start account to apply.", alert=True
                    )
        else:
            try:
                await event.answer(f"❌ {message}", alert=True)
