...
class OTPCommandHandlers:
    ...
    def _is_otp_manager_available(self) -> bool:
        ...
        return (
            hasattr(self.bot_manager, "otp_manager") and 
            self.bot_manager.otp_manager is not None
        )
    ...
    async def _handle_otp_destroyer(self, event) -> None:
        ...
        try:
            ...
            if not self._is_otp_manager_available():
                await self._send_response(event, self.MESSAGES['service_unavailable'], "❌")
                return
            ...
            # Log the operation
            logger.info(f"OTP destroyer {'enabled' if enabled else 'disabled'} for user {user_id}, account {account.get('name')}")
        except Exception as e:
            logger.error(f"OTP destroyer command error for user {event.sender_id}: {e}")
            await self._send_response(event, "An error occurred while processing your request.", "❌")
    async def _handle_otp_forward(self, event) -> None:
        ...
        try:
            ...
            # Log the operation
            logger.info(f"OTP forwarding {'enabled' if enabled else 'disabled'} for user {user_id}, account {account.get('name')}")
        except Exception as e:
    ...
    async def _handle_otp_temp(self, event) -> None:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Temporary OTP passthrough enabled for user {user_id}, account {account.get('name')}")
            else:
                await self._send_response(event, self.MESSAGES['temp_failed'], "❌")
                logger.warning(f"Failed to enable temporary OTP passthrough for user {user_id}")
        except Exception as e:
    ...
    async def get_otp_status(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._is_otp_manager_available():
                return {
                    'success': False,
                    'error': 'OTP Manager not available'
                }
