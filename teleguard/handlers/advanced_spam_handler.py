...
class AdvancedSpamHandler:
    ...
    async def _handle_spam_master_menu(self, event):
        ...
        if not warning_accepted:
            buttons = [
                [
                    Button.inline(
                        "✅ I Accept All Terms & Risks", b"accept_spam_warning"
                    )
                ],
        ...
        buttons = [
            [
                Button.inline("📤 Mass Inviter", b"mass_invite"),
                Button.inline("📇 Contact Scraper", b"contact_scrape"),
            ],
            [
                Button.inline("🔍 Username Checker", b"username_check"),
                Button.inline("💣 Forward Bomber", b"forward_bomb"),
            ],
            [
                Button.inline("🌊 Message Flooder", b"message_flood"),
                Button.inline("⚔️ Raid Coordinator", b"raid_coord"),
            ],
            [
                Button.inline("🥷 Stealth Raid", b"stealth_raid"),
                Button.inline("🎯 Multi-Target Raid", b"multi_raid"),
            ],
        ...
        try:
            ...
            await self.bot.send_message(
                user_id, f"❌ Error loading SpamMaster menu: {str(e)}"
            )
    ...
    def _register_warning_handlers(self):
        ...
        async def accept_warning(event):
            ...
            await mongodb.db.spam_config.update_one(
                {"user_id": user_id}, {"$set": {"warning_accepted": True}}, upsert=True
            )
        ...
        async def decline_warning(event):
            ...
            if hasattr(self.bot_manager, "menu_system"):
                await self.bot_manager.menu_system._handle_help(
                    type(
                        "Event",
                        (),
                        {
                            ...
                            "reply": lambda x, buttons=None: self.bot.send_message(
                                event.sender_id, x, buttons=buttons
                            ),
                        },
                    )()
                )
    ...
    def _register_mass_invite_handlers(self):
        ...
        async def mass_invite_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"mi_acc:{acc['phone']}".encode())]
    ...
    def _register_contact_scrape_handlers(self):
        ...
        async def contact_scrape_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"cs_acc:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
        ...
        async def contact_scrape_all_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"cs_all:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
    ...
    def _register_username_check_handlers(self):
        ...
        async def username_check_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"uc_acc:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
            buttons.append([Button.inline("🔙 Back", b"advanced_spam")])
    ...
    def _register_forward_bomb_handlers(self):
        ...
        async def forward_bomb_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"fb_acc:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
            buttons.append([Button.inline("🔙 Back", b"advanced_spam")])
    ...
    def _register_message_flood_handlers(self):
        ...
        async def message_flood_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"mf_acc:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
            buttons.append([Button.inline("🔙 Back", b"advanced_spam")])
    ...
    def _register_send_all_groups_handlers(self):
        ...
        async def send_all_groups_menu(event):
            ...
            buttons = [
                [Button.inline(f"📱 {acc['name']}", f"sag_acc:{acc['phone']}".encode())]
                for acc in accounts[:10]
            ]
            buttons.append([Button.inline("🔙 Back", b"advanced_spam")])
    ...
    async def _get_accounts(self, user_id):
        ...
        return [
            {
                "phone": acc["phone"],
                "name": acc.get("name", acc.get("first_name", acc["phone"])),
            }
            for acc in accounts
        ]
    ...
    async def _execute_mass_invite(self, event, temp_data):
        ...
        try:
            ...
            users = (
                await mongodb.db.spam_users.find({"owner_id": user_id})
                .limit(50)
                .to_list(None)
            )
            ...
            await msg.edit(
                f"✅ Mass invite completed!\n\nInvited: {invited}\nFailed: {failed}"
            )
        except Exception as e:
            await msg.edit(f"❌ Error: {e}")
        finally:
            await mongodb.db.temp_data.delete_one(
                {"user_id": user_id, "type": "mass_invite"}
            )
    ...
    async def _execute_contact_scrape(self, event, temp_data):
        ...
        try:
            ...
            await self.bot.send_file(
                user_id, filename, caption=f"📇 Scraped {len(contacts)} contacts"
            )
    ...
    async def _execute_contact_scrape_all(self, event, phone, user_id):
        msg = await self.bot.send_message(
            user_id, "🌐 Scraping contacts from all groups..."
        )
        ...
        try:
            ...
            await self.bot.send_file(
                ...
                filename,
                caption=f"🌐 Scraped {
                    len(contacts)} unique contacts from {groups_scraped} groups",
            )
            await msg.edit(
                f"✅ All groups scraped!\n\nGroups: {groups_scraped}\nContacts: {
                    len(contacts)}\n\nFile sent above."
            )
        except Exception as e:
            await msg.edit(f"❌ Error: {e}")
    ...
    async def _execute_raid_coord(self, event, temp_data):
        ...
        try:
            accounts = await self._get_accounts(user_id)
            clients = [
                self._get_client(acc["phone"])
                for acc in accounts
                if self._get_client(acc["phone"])
            ]
    ...
    async def _execute_multi_raid(self, event, temp_data):
        ...
        msg = await event.reply(
            f"🎯 Starting multi-target raid on {len(targets)} groups..."
        )
    ...
    async def _execute_mass_invite_all(self, event, phone, user_id):
        msg = await self.bot.send_message(
            user_id, "🌐 Starting mass invite to all groups..."
        )
            ...
            for group in groups:
                try:
                    for user in users:
                        try:
                            await client(
                                InviteToChannelRequest(group.entity, [user["user_id"]])
                            )
                            total_invited += 1
                            await asyncio.sleep(random.randint(5, 15))
                        except BaseException:
