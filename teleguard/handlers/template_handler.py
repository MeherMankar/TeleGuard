...
class TemplateHandler:
    ...
    async def start_text_template_creation(self, event):
        ...
        user_id = event.sender_id
        self.pending_actions[user_id] = {"type": "template_create_text", "step": "name"}
    ...
    async def show_view_menu(self, event):
        ...
        for category in categories:
            category_templates = await self.messaging_manager.get_templates_by_category(
                user_id, category
            )
            buttons.append(
                [
                    Button.inline(
                        f"📁 {category} ({len(category_templates)})",
                        f"template:view_category:{category}",
                    )
                ]
            )
    ...
    async def show_templates_by_category(self, event, category):
        ...
        user_id = event.sender_id
        templates = await self.messaging_manager.get_templates_by_category(
            user_id, category
        )
        ...
        for template in templates[:10]:  # Limit to 10 for UI
            ...
            buttons.append(
                [
                    Button.inline(
                        f"📝 {template['name']}", f"template:edit:{template['_id']}"
                    )
                ]
            )
        if not templates:
    ...
    async def delete_template(self, event, template_id):
        ...
        if success:
            await event.edit(
                ...
                buttons=[[Button.inline("🔙 Back", "template:view")]],
            )
        else:
            await event.edit(
        ...
        for account in accounts:
            buttons.append(
                [
                    Button.inline(
                        f"📱 {account['name']}",
                        f"template:select_account:{account['name']}",
                    )
                ]
            )
        ...
        if action["step"] == "name":
            ...
            if text.lower() != "skip":
                try:
                    ...
                    await event.reply(
                        f"❌ Invalid format: {str(e)}. Try again or type 'skip':"
                    )
                    return
                except Exception:
                    await event.reply(
                        "❌ Invalid JSON format. Try again or type 'skip':"
                    )
                    return
            try:
                ...
                del self.pending_actions[user_id]
                await event.reply(
                    f"✅ Text template '{action['name']}' created successfully!",
                    buttons=[[Button.inline("🔙 Back", "template:main")]],
                )
            except Exception as e:
    ...
    async def process_media_template_input(self, event, action, text):
        ...
        if action["step"] == "name":
            ...
            action["step"] = "content"
            await event.reply(
                "Step 4/5: Enter caption/content:\n\nAvailable variables: {name}, {username}, {time}, {date}"
            )
        elif action["step"] == "content":
            action["content"] = text
            action["step"] = "buttons"
            await event.reply(
                'Step 5/5: Enter buttons (JSON format) or \'skip\':\n\nExample: [{"text": "Visit", "url": "https://example.com"}]'
            )
        elif action["step"] == "buttons":
            buttons = []
            if text.lower() != "skip":
            ...
            try:
                ...
                del self.pending_actions[user_id]
                await event.reply(
                    f"✅ Media template '{action['name']}' created successfully!",
                    buttons=[[Button.inline("🔙 Back", "template:main")]],
                )
            except Exception as e:
                await event.reply(f"❌ Failed to create template: {e}")
    ...
    async def get_template_selection_menu(self, user_id: int) -> tuple[str, list]:
        ...
        for template in templates[:10]:  # Limit to 10
            ...
            buttons.append(
                [
                    Button.inline(
                        f"📝 {template['name']}", f"template:use:{template['_id']}"
                    )
                ]
            )
