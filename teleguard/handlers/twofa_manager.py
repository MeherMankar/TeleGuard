...
class TwoFAManager:
    ...
    async def change_2fa_password(
        self, user_id: int, account_id: str, current_password: str, new_password: str
    ) -> bool:
        ...
        try:
            ...
            if not await self.verify_current_password(
        ...
        try:
            ...
            await client(
                functions.account.UpdatePasswordSettingsRequest(
                    ...
                    new_settings=functions.account.PasswordInputSettings(
                        new_algo=None, new_password_hash=b"", hint=""
                    ),
                )
            )
            ...
            await mongodb.db.accounts.update_one(
                {"_id": ObjectId(account_id)}, {"$unset": {"twofa_password_hash": ""}}
            )
