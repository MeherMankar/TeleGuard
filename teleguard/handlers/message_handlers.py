...
class MessageHandlers:
    ...
    async def _process_add_account(self, event, user_id, phone):
        ...
        try:
            ...
            # Add OTP protection for this phone number to prevent destroyer from
            # invalidating legitimate codes
            try:
    ...
    async def _process_verify_otp(self, event, user_id, code, user):
        ...
        try:
            ...
            try:
                await mongodb.db.otp_protections.delete_one(
                    {"phone": phone, "reason": "account_addition"}
                )
                logger.info(
                    f"Removed OTP protection for {phone} after successful addition"
                )
            except Exception as e:
            ...
            if "expired" in error_msg.lower():
                ...
                try:
                    ...
                    logger.info(
                        f"Removed OTP protection for {phone} after auth failure"
                    )
                except Exception:
                    pass
                self.pending_actions.pop(user_id, None)
    ...
    async def _process_verify_2fa(self, event, user_id, password, user):
        ...
        try:
            ...
            if "attempts left" in error_msg or "Try again in" in error_msg:
                ...
                await event.reply(
                    f"❌ 2FA failed: {error_msg if error_msg else 'Unknown error'}"
                )
    ...
    async def _handle_2fa_management_actions(self, event, user, action, message):
        ...
        if action == "change_2fa_current":
            ...
            if hasattr(self.bot_manager, "twofa_manager"):
                is_valid = await self.bot_manager.twofa_manager.verify_current_password(
                ...
                if is_valid:
                    ...
                    await self.bot.send_message(
                        user_id, "❌ Current password is incorrect"
                    )
                    self.pending_actions.pop(user_id, None)
            else:
            ...
            if hasattr(self.bot_manager, "twofa_manager"):
                ...
                if success:
                    await self.bot.send_message(
                        user_id, "✅ 2FA password changed successfully!"
                    )
                else:
                    await self.bot.send_message(
                        user_id, "❌ Failed to change 2FA password"
                    )
            else:
                await self.bot.send_message(user_id, "❌ 2FA management not available")
            ...
            if hasattr(self.bot_manager, "twofa_manager"):
                ...
                if success:
                    await self.bot.send_message(
                        user_id,
                        "✅ 2FA password removed successfully!\n\n⚠️ Your account is now less secure.",
                    )
                else:
                    await self.bot.send_message(
                        user_id,
                        "❌ Failed to remove 2FA password. Check your password.",
                    )
            else:
                await self.bot.send_message(user_id, "❌ 2FA management not available")
            ...
            if hasattr(self.bot_manager, "twofa_manager"):
                ...
                if success:
                    await self.bot.send_message(
                        user_id, "✅ 2FA password set successfully!"
                    )
                else:
                    await self.bot.send_message(
                        user_id, "❌ Failed to set 2FA password"
                    )
            else:
                await self.bot.send_message(user_id, "❌ 2FA management not available")
    ...
    async def _get_or_reconnect_client(self, user_id, account):
        ...
        if session_string and phone:
            try:
                ...
                account_name = (
                    account.get("name") or account.get("display_name") or phone
                )
    ...
    async def _handle_messaging_actions(self, event, user, action, message):
        ...
        if action == "compose_message_target":
            ...
            if message.lower() == "template":
                ...
                text, buttons = (
                    await self.bot_manager.template_handler.get_template_selection_menu(
                        user_id
                    )
                )
    ...
    async def _handle_session_string_import(self, event, user, action, message):
        ...
        if (
            hasattr(self.bot_manager, "session_login_handler")
            and action == "session_string_login"
        ):
            success, msg = (
                await self.bot_manager.session_login_handler.process_session_string(
                    user_id, session_string
                )
            )
            await event.reply(msg)
        elif (
            hasattr(self.bot_manager, "session_import_handler")
            and action == "import_string_session"
        ):
            success, msg = (
                await self.bot_manager.session_import_handler.process_string_session(
                    user_id, session_string
                )
            )
            await event.reply(msg)
        ...
        if hasattr(self.bot_manager, "session_login_handler"):
            ...
            if user_id in self.bot_manager.session_login_handler.pending_auth:
                ...
                if auth_data.get("step") == "2fa":
                    success, msg = (
                        await self.bot_manager.session_login_handler.process_2fa_password(
                ...
                success, msg = (
                    await self.bot_manager.session_login_handler.process_phone_login(
                        user_id, message.strip()
                    )
                )
            await event.reply(msg)
    ...
    async def _handle_session_validation(self, event, user, action, message):
        ...
        try:
            ...
            is_valid, result = await validate_string_session(
            ...
            if is_valid:
                ...
                try:
                    ...
                    if len(session_string) > 50:
                        # This is a rough estimation - actual DC detection would require
                        # parsing
    ...
    async def _handle_cleanup_selection(self, event, user, action, message):
        ...
        user_id = event.sender_id
        logger.info(
            f"Cleanup selection handler called for user {user_id}, message: {message}"
        )
        ...
        logger.info(
            f"Validated cleanup types: {selected_types} for account {account_id}"
        )
    ...
    async def _handle_bulk_cleanup_selection(self, event, user, action, message):
        ...
        try:
            if hasattr(self.bot_manager, "menu_system") and hasattr(
                ...
                await self.bot_manager.menu_system.cleanup_operations.execute_bulk_cleanup(
        ...
        try:
            if user_id in self.user_clients and phone in self.user_clients[user_id]:
                ...
                if client and client.is_connected():
                    ...
                    logger.info(
                        f"Updated account info for {phone}: {display_name} (ID: {telegram_id})"
                    )
    ...
    async def _handle_session_creation_2fa(self, event, user, action, message):
        ...
        try:
            ...
            if not hasattr(self.bot_manager, "session_login_handler"):
                await self.bot.send_message(
                    user_id, "❌ Session login handler not available"
                )
                self.pending_actions.pop(user_id, None)
                return
            ...
            if user_id not in session_handler.pending_auth:
                await self.bot.send_message(
                    user_id, "❌ Session creation expired. Please try again."
                )
                self.pending_actions.pop(user_id, None)
                return
            ...
            if not client or not phone:
                await self.bot.send_message(
                    user_id, "❌ Invalid session state. Please try again."
                )
                self.pending_actions.pop(user_id, None)
            ...
            try:
                ...
                if account:
                    await db_manager.store_2fa_password(
                        user_id, str(account["_id"]), password
                    )
                ...
                error_msg = str(e)
                if (
                    "PASSWORD_HASH_INVALID" in error_msg
            ...
            await self.bot.send_message(
                user_id, f"❌ Error processing 2FA password: {str(e)}"
            )
            self.pending_actions.pop(user_id, None)
            if hasattr(self.bot_manager, "session_login_handler"):
    ...
    async def _handle_add_proxy(self, event, user, action, message):
        ...
        if hasattr(self.bot_manager, "menu_system") and hasattr(
            self.bot_manager.menu_system, "proxy_handler"
        ):
            await self.bot_manager.menu_system.proxy_handler.process_proxy_input(
                event, user_id, message.strip()
            )
        else:
