...
class DeviceHandler:
    ...
    async def _select_account(self, event, action: str):
        ...
        if not accounts:
            await event.edit(
                ...
                buttons=[[Button.inline("🔙 Back", "device:menu")]],
            )
            return
        buttons = []
        for acc in accounts:
            ...
            buttons.append(
                [
                    Button.inline(
                        f"📱 {display_name}", f"device:{action}_account:{acc['_id']}"
                    )
                ]
            )
    ...
    async def scan_devices(self, event, account_id=None):
        ...
        if "error" in result:
            ...
            return
        text = f"🔍 **Active Devices for {
            format_display_name(account)}** ({
            result['count']} found)\n\n"
        if not result["devices"]:
            ...
            for dev in result["devices"]:
                ...
                text += f"{device_emoji} **{
                    dev.get(
                        'device_model',
                        'Unknown')}**{current_marker}\n"
                text += f"   OS: {dev.get('os_name',
                                          'N/A')} {dev.get('os_version',
                                                           'N/A')}\n"
                text += f"   App: {dev.get('app_name',
                                           'N/A')} {dev.get('app_version',
                                                            'N/A')}\n"
                text += f"   IP: {dev.get('ip', 'N/A')} ({dev.get('country', 'N/A')})\n"
                if dev.get("date_active"):
                    text += f"   Last Active: {
                        dev['date_active'].strftime('%Y-%m-%d %H:%M')}\n\n"
                else:
                    text += "\n"
        buttons = [
    ...
    async def show_suspicious_devices(self, event, account_id=None):
        ...
        text = f"⚠️ **Suspicious Devices for {
            format_display_name(account)}** ({
            len(suspicious_devices)} found)\n\n"
    ...
    def _get_device_emoji(self, device_type: str) -> str:
        ...
        emoji_map = {"Mobile": "📱", "Tablet": "📲", "Unknown": "❓"}
