...
class DMReplyHandler:
    ...
    async def _setup_client_dm_handler(self, user_id: int, account_name: str, client):
        ...
        try:
            if not client or not client.is_connected():
                logger.warning(
                    f"Client {account_name} is not connected, skipping DM handler setup"
                )
                return
            ...
            if client_key in self.handled_clients:
                logger.info(
                    f"DM handler already exists for {account_name}, removing old handler"
                )
            ...
            async def dm_handler(event):
                try:
                    ...
                    account_display_name = (
                        ...
                        else me.first_name or f"ID: {me.id}"
                    )
                    ...
                    if admin_group_id:
                        ...
                        topic_id = await self._get_or_create_topic(
                            admin_group_id, topic_title, sender.id, me.id
                        )
                        ...
                        try:
                            if topic_id:
                                ...
                                if event.media:
                                    await self.bot.send_file(
                                        admin_group_id, event.media, caption=event.text
                                    )
                                else:
                                    await self.bot.send_message(
                                        admin_group_id, event.text or "[Empty]"
                                    )
                        except ValueError as ve:
                            if "Could not find the input entity" in str(ve):
                                logger.warning(
                                    f"Cannot access admin group {admin_group_id}, removing from user settings"
                                )
                            ...
                            if (
                                "topic was deleted" in error_msg.lower()
                                or "TOPIC_DELETED" in error_msg
                            ):
                                logger.info(
                                    f"Topic {topic_id} was deleted, recreating..."
                                )
                                # Remove old mapping
                                await mongodb.db.dm_topics.delete_one(
                                    {"topic_id": topic_id}
                                )
                                # Create new topic
                                new_topic_id = await self._get_or_create_topic(
                                    admin_group_id, topic_title, sender.id, me.id
                                )
                                if new_topic_id:
                                    # Retry sending
            ...
            try:
                await BotLogger.log_error(
                    "DM Setup Error", str(e)[:500], user_id, f"Account: {account_name}"
                )
            except BaseException:
                pass
        ...
        if self.ai_model:
            try:
                await self.setup_ai_automation_job(
                    user_id,
                    str(me.id),
                    {
    ...
    async def _get_or_create_topic(
        self, group_id: int, topic_title: str, sender_id: int, account_id: int
    ):
        ...
        try:
            ...
            existing_topic = await mongodb.db.dm_topics.find_one(
                {"group_id": group_id, "topic_key": topic_key}
            )
            ...
            try:
                await BotLogger.log_error(
                    ...
                    str(e)[:500],
                    context=f"Group: {group_id}, Title: {topic_title}",
                )
            except BaseException:
                pass
        return None

    async def setup_ai_automation_job(
        self, user_id: int, account_id: str, config: dict
    ):
    ...
    async def refresh_all_handlers(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Refreshed DM handlers: {handler_count} accounts registered"
            )
        except Exception as e:
    ...
    async def _handle_ai_auto_reply(self, event, me, user_id: int):
        ...
        try:
            ...
            if response:
                ...
                await self._simulate_human_auto_reply_behavior(
                    event.client, event.chat_id, response
                )
