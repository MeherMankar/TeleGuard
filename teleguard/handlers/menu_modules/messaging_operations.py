...
class MessagingOperations:
    ...
    async def send_autoreply_menu(self, user_id, message_id):
        ...
        if not accounts:
            ...
            text = f"🤖 **Advanced Auto-Reply System**\n\n📱 Accounts: {enabled_count}/{len(accounts)} enabled\n🔑 Keyword Replies: {keyword_status}\n⏰ Time-based Replies: {time_status}\n\nConfigure your automatic responses:"
            buttons = [[Button.inline("📱 Toggle Per Account", "auto_reply:toggle")], [Button.inline("🔑 Keyword Settings", "auto_reply:keyword_settings")], [Button.inline("⏰ Time Settings", "auto_reply:time_settings")], [Button.inline("📊 View Stats", "auto_reply:analytics")], [Button.inline("🗑️ Reset All", "auto_reply:reset")], [Button.inline("🔙 Back", "menu:messaging")]]
        await self.bot.edit_message(user_id, message_id, text, buttons=buttons)
    ...
    async def send_templates_menu(self, user_id, message_id):
        ...
        buttons = [[Button.inline("🚀 Open Advanced Templates", "template:main")], [Button.inline("🔙 Back", "menu:messaging")]]
        await self.bot.edit_message(user_id, message_id, text, buttons=buttons)
    ...
    async def start_bulk_all_flow(self, user_id, event):
        ...
        if self.account_manager:
            self.account_manager.pending_actions[user_id] = {"action": "bulk_all_targets"}
