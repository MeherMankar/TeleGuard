...
class MenuHandlers:
    ...
    async def handle_account_settings(self, event):
        ...
        if not accounts:
            ...
            buttons = [[self.bot.Button.inline("🚀 Add First Account", "account:add")], [self.bot.Button.inline("🔐 Session Login", "session_login"), self.bot.Button.inline("📥 Import Sessions", "import_sessions")], [self.bot.Button.inline("❓ Setup Guide", "help:guide")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        else:
            ...
            for i, account in enumerate(accounts, 1):
                ...
                buttons.append([self.bot.Button.inline(f"⚙️ {format_display_name(account)}", f"account:manage:{account['_id']}")])
            text += "\n🎛️ **Management Tools:**"
            buttons.extend([[self.bot.Button.inline("➕ Add Account", "account:add"), self.bot.Button.inline("🗑️ Remove Account", "account:remove")], [self.bot.Button.inline("🔐 Login via Session", "session_login"), self.bot.Button.inline("✨ Create Session", "export_sessions")], [self.bot.Button.inline("🔄 Refresh Status", "account:refresh"), self.bot.Button.inline("📋 Detailed List", "account:list")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]])
    ...
    async def handle_otp_manager(self, event):
        ...
        if not accounts:
            ...
            buttons = [[self.bot.Button.inline("🚀 Add First Account", "account:add")], [self.bot.Button.inline("❓ Security Guide", "help:security")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        else:
            ...
            text = f"🛡️ **OTP Security Manager**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n📊 **Security Dashboard:**\n• {security_emoji} **Security Score:** {security_score}%\n• 🛡️ **Destroyer Active:** {destroyer_enabled}/{len(accounts)} accounts\n• 📤 **Forward Active:** {forward_enabled}/{len(accounts)} accounts\n• ⏰ **Temp Bypass:** {temp_active} active\n\n🎛️ **Protection Controls:**\nChoose your security configuration below:"
            buttons = [[self.bot.Button.inline("🛡️ OTP Destroyer", "otp_setting:destroyer"), self.bot.Button.inline("📤 OTP Forward", "otp_setting:forward")], [self.bot.Button.inline("⏰ Temp Bypass", "otp_setting:temp"), self.bot.Button.inline("📊 Statistics", "otp:stats")], [self.bot.Button.inline("🟢 Enable All Protection", "otp:enable_all"), self.bot.Button.inline("🔴 Disable All Protection", "otp:disable_all")], [self.bot.Button.inline("📋 Security Audit Log", "otp:audit_all")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
            await self.bot.send_message(user_id, text, buttons=buttons)
    ...
    async def handle_messaging(self, event):
        ...
        if not accounts:
            ...
            buttons = [[self.bot.Button.inline("🚀 Add First Account", "account:add")], [self.bot.Button.inline("❓ Messaging Guide", "help:features")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        else:
            active_accounts = sum(1 for acc in accounts if acc.get("is_active", False))
            ...
            buttons = [[self.bot.Button.inline("📤 Smart Messaging", "msg:send"), self.bot.Button.inline("📨 Bulk Campaigns", "msg:bulk")], [self.bot.Button.inline("🤖 Auto-Reply System", "auto_reply:main"), self.bot.Button.inline("📝 Message Templates", "msg:templates")], [self.bot.Button.inline("📨 Unified DM Manager", "dm_reply:main"), self.bot.Button.inline("📊 Analytics", "msg:stats")], [self.bot.Button.inline("📋 Message History", "msg:history"), self.bot.Button.inline("⚙️ System Settings", "msg:settings")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
            await self.bot.send_message(user_id, text, buttons=buttons)
    ...
    async def handle_channels(self, event):
        ...
        if not accounts:
            ...
            buttons = [[self.bot.Button.inline("🚀 Add First Account", "account:add")], [self.bot.Button.inline("❓ Channel Guide", "help:features")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        else:
            active_accounts = sum(1 for acc in accounts if acc.get("is_active", False))
            ...
            for account in accounts[:8]:
                ...
                buttons.append([self.bot.Button.inline(button_text, f"channel:select:{account['phone']}")])
            buttons.extend([[self.bot.Button.inline("📊 Global Statistics", "channel:stats"), self.bot.Button.inline("🔍 Channel Discovery", "channel:search")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]])
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
            await self.bot.send_message(user_id, text, buttons=buttons)
    ...
    async def handle_cleanup(self, event):
        ...
        if not accounts:
            ...
            buttons = [[self.bot.Button.inline("🚀 Add First Account", "account:add")], [self.bot.Button.inline("❓ Cleanup Guide", "help:features")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        else:
            active_accounts = sum(1 for acc in accounts if acc.get("is_active", False))
            ...
            for account in accounts:
                ...
                display_name = format_display_name(account)
                button_text = f"{status} {display_name}"
                buttons.append([self.bot.Button.inline(button_text, f"cleanup:select:{account['_id']}")])
            buttons.append([self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")])
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
            await self.bot.send_message(user_id, text, buttons=buttons)
    ...
    async def handle_help(self, event):
        ...
        buttons = [[self.bot.Button.inline("📖 Complete Guide", "help:guide"), self.bot.Button.inline("🛡️ Security Guide", "help:security")], [self.bot.Button.inline("⚙️ Feature Guide", "help:features"), self.bot.Button.inline("🔧 Troubleshooting", "help:troubleshoot")], [self.bot.Button.inline("❓ FAQ", "help:faq"), self.bot.Button.inline("📞 Contact Support", "help:contact")], [self.bot.Button.inline("🆘 Emergency Help", "help:emergency"), self.bot.Button.inline("📚 Commands", "help:commands")], [self.bot.Button.inline("📨 Chat Import", "menu:import")]]
        if user and user_id in ADMIN_IDS:
            ...
            buttons.append([self.bot.Button.inline(dev_text, "help:toggle_dev")])
        buttons.append([self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")])
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
    ...
    async def handle_support(self, event):
        ...
        buttons = [[self.bot.Button.inline("💬 Contact Support", "support:contact"), self.bot.Button.inline("🐛 Report Bug", "support:bug")], [self.bot.Button.inline("📚 Documentation", "support:docs"), self.bot.Button.inline("💡 Feature Request", "support:feature")], [self.bot.Button.inline("📊 System Status", "support:status"), self.bot.Button.inline("🔄 Updates", "support:updates")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
        if hasattr(event, 'message_id'):
            await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
        else:
            await self.bot.send_message(user_id, text, buttons=buttons)
    ...
    async def handle_developer(self, event):
        ...
        if user:
            ...
            buttons = [[self.bot.Button.inline(mode_text, "dev:toggle")], [self.bot.Button.inline("📊 System Dashboard", "dev:sysinfo"), self.bot.Button.inline("📋 System Logs", "dev:logs")], [self.bot.Button.inline("🗄️ Database Tools", "dev:dbstats"), self.bot.Button.inline("⚡ Performance Monitor", "dev:perf")], [self.bot.Button.inline("🔧 Maintenance Tools", "dev:maintenance"), self.bot.Button.inline("🔄 System Restart", "dev:restart")], [self.bot.Button.inline("🚀 Startup Config", "dev:startup"), self.bot.Button.inline("📚 Command Reference", "dev:commands")], [self.bot.Button.inline("🔙 Back to Main Menu", "menu:main")]]
            if hasattr(event, 'message_id'):
                await self.bot.edit_message(user_id, event.message_id, text, buttons=buttons)
            else:
                await self.bot.send_message(user_id, text, buttons=buttons)
