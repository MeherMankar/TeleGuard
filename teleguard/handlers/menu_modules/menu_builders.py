...
class MenuBuilders:
    ...
    def get_otp_account_buttons(self, account_id, account):
        destroyer_enabled = account.get("otp_destroyer_enabled", False) if isinstance(account, dict) else getattr(account, "otp_destroyer_enabled", False)
        forward_enabled = account.get("otp_forward_enabled", False) if isinstance(account, dict) else getattr(account, "otp_forward_enabled", False)
        has_password = account.get("otp_destroyer_disable_auth") if isinstance(account, dict) else getattr(account, "otp_destroyer_disable_auth", None)
        destroyer_text = "❌ Disable Destroyer" if destroyer_enabled else "✅ Enable Destroyer"
        ...
        forward_action = f"otp:forward_disable:{account_id}" if forward_enabled else f"otp:forward_enable:{account_id}"
        import time
        temp_active = False
        ...
        if has_password:
            buttons.append([Button.inline("🛡️ Change Password", f"otp_pwd:change:{account_id}"), Button.inline("❌ Remove Password", f"otp_pwd:remove:{account_id}")])
        else:
