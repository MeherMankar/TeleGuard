    ...
    def register_handlers(self):
                ...
                for account in accounts:
                    if not any(key.endswith("_enc") for key in account.keys()):
                        ...
                        await mongodb.db.accounts.replace_one(
                            {"_id": account["_id"]}, encrypted_data
                        )
                        account_count += 1
                await event.reply(
        ...
        async def cache_stats_handler(event):
            ...
            try:
                ...
                if not stats:
                    await event.reply(
                        "❌ Cache statistics unavailable (Redis disconnected)"
                    )
                    return
                ...
                message += (
                    f"🔹 **Connected Clients:** {stats.get('connected_clients', 0)}\n"
                )
                message += f"🔹 **Total Commands:** {
                    stats.get(
        ...
        async def cache_warm_handler(event):
            ...
            try:
                ...
                await event.reply(
                    f"✅ Cache warmed successfully for user {target_user_id}"
                )
            except ValueError:
                await event.reply(
        ...
        async def cache_clear_handler(event):
            ...
            if user_id not in ADMIN_IDS:
                ...
                return
            await event.reply(
                "⚠️ **WARNING**: This will clear ALL cache data!\n\nReply with 'CONFIRM' to proceed."
            )

            async def confirmation_handler(confirm_event):
                if (
                    confirm_event.sender_id == event.sender_id
                    and confirm_event.text == "CONFIRM"
                ):
                    try:
        ...
        async def cache_health_handler(event):
            ...
            try:
                ...
                if health.get("cache_stats"):
                    ...
                    message += (
                        f"   • Clients: {cache_stats.get('connected_clients', 0)}\n"
                    )
                await event.reply(message)
            except Exception as e:
        ...
        async def backup_status_handler(event):
            ...
            try:
                ...
                status_text += f"Session Backup: {
                    '✅ Enabled' if SESSION_BACKUP_ENABLED else '❌ Disabled'}\n"
                status_text += f"Telegram Channel: {
                    '✅ Configured' if TELEGRAM_BACKUP_CHANNEL else '❌ Not Set'}\n"
                status_text += f"Scheduler: {
                    '✅ Running' if self.session_scheduler and self.session_scheduler.running else '❌ Not Running'}\n\n"
