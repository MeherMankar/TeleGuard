...
class SpamFiltersHandler:
    ...
    def register_handlers(self):
        ...
        async def delays_menu(event):
            ...
            buttons = [
                [
                    Button.inline(
                        f"{'✅' if config.get('smart_delays') else '☐'} Smart Delays",
                        b"toggle_smart_delays",
                    )
                ],
        ...
        async def toggle_delays(event):
            ...
            await mongodb.db.spam_config.update_one(
                {"user_id": user_id}, {"$set": {"smart_delays": enabled}}, upsert=True
            )

            await event.answer(
                f"Smart Delays {'Enabled' if enabled else 'Disabled'}", alert=True
            )
        ...
        async def users_filter_menu(event):
            ...
            buttons = [
                [
                    Button.inline(
                        f"{'✅' if 'premium' in filters else '☐'} Premium Only",
                        b"toggle_premium",
                    )
                ],
                [
                    Button.inline(
                        f"{'✅' if 'verified' in filters else '☐'} Verified Only",
                        b"toggle_verified",
                    )
                ],
                [
                    Button.inline(
                        f"{'✅' if 'active' in filters else '☐'} Active Users",
                        b"toggle_active",
                    )
                ],
                [
                    Button.inline(
                        f"{'✅' if 'no_bots' in filters else '☐'} Exclude Bots",
                        b"toggle_bots",
                    )
                ],
        ...
        async def blacklist_menu(event):
            ...
            count = await mongodb.db.spam_blacklist.count_documents(
                {"owner_id": user_id}
            )
        ...
        async def settings_menu(event):
            ...
            buttons = [
                [
                    Button.inline(
                        f"Max Users: {config.get('max_users', 500)}", b"set_max_users"
                    )
                ],
                [
                    Button.inline(
                        f"{'✅' if config.get('skip_sent') else '☐'} Skip Sent",
                        b"toggle_skip_sent",
                    )
                ],
                [Button.inline("🔙 Back", b"spam_filters")],
        ...
        async def toggle_skip(event):
            ...
            await mongodb.db.spam_config.update_one(
                {"user_id": user_id}, {"$set": {"skip_sent": enabled}}, upsert=True
            )

            await event.answer(
                f"Skip Sent {'Enabled' if enabled else 'Disabled'}", alert=True
            )
        ...
        async def handle_filter_inputs(msg_event):
            ...
            delay_state = await mongodb.db.temp_data.find_one(
                {"user_id": user_id, "type": "delay_wait"}
            )
            if delay_state:
                try:
            ...
            # Handle blacklist
            blacklist_state = await mongodb.db.temp_data.find_one(
                {"user_id": user_id, "type": "blacklist_wait"}
            )
            if blacklist_state:
                try:
    ...
    async def _toggle_filter(self, event, filter_name: str):
        ...
        await mongodb.db.spam_config.update_one(
            {"user_id": user_id}, {"$set": {"filters": filters}}, upsert=True
        )

        await event.answer(
            f"{filter_name.title()} {'Enabled' if filter_name in filters else 'Disabled'}",
            alert=True,
        )
