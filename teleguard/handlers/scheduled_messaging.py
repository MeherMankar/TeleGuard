...
class ScheduledMessaging:
    ...
    def register_handlers(self):
        ...
        self.bot.add_event_handler(
            ...
            events.CallbackQuery(pattern=r"^schedule_msg$")
        )
        self.bot.add_event_handler(
            self._list_scheduled,
            events.CallbackQuery(pattern=r"^schedule_list$")
        )
        self.bot.add_event_handler(
    ...
    async def _list_scheduled(self, event):
        ...
        try:
            scheduled = await mongodb.db.scheduled_messages.find(
    ...
    async def _send_scheduled_message(self, msg_data):
        ...
        try:
            ...
            await self.bot.send_message(
                user_id,
                f"✅ Scheduled message sent to {recipient}"
            )
        except Exception as e:
